...
func (s *Store) Register(email, password string) (User, string, error) {
	...
	id, _ := r.LastInsertId()
	token, th := Token()
	_, e = s.DB.Exec("INSERT INTO sessions VALUES(?,?,?)", th, id, time.Now().Add(7*24*time.Hour).UTC().Format(time.RFC3339))
...
func (s *Store) Login(email, password string) (User, string, error) {
	...
	token, th := Token()
	_, e = s.DB.Exec("INSERT INTO sessions VALUES(?,?,?)", th, u.ID, time.Now().Add(7*24*time.Hour).UTC().Format(time.RFC3339))
	return u, token, e
}
...
func (s *Store) CreateSMS(u User, o SMSOrder) error {
	...
	_, e = tx.Exec(`INSERT INTO sms_orders(id,user_id,country,service,status,upstream_cost,price_fen,created_at)VALUES(?,?,?,?,?,?,?,?)`, o.ID, u.ID, o.Country, o.Service, "purchasing", o.UpstreamCost, o.PriceFen, o.CreatedAt)
	if e != nil {
		return e
	}
...
func (s *Store) ActivateSMS(id, upstream, phone string, cost float64) error {
	_, e := s.DB.Exec("UPDATE sms_orders SET upstream_id=?,phone=?,upstream_cost=?,status='waiting' WHERE id=?", upstream, phone, cost, id)
	return e
}
...
func (s *Store) GetSMS(id string, uid int64) (SMSOrder, error) {
	var o SMSOrder
	e := s.DB.QueryRow(`SELECT id,user_id,COALESCE(upstream_id,''),country,service,COALESCE(phone,''),status,COALESCE(code,''),upstream_cost,price_fen,created_at FROM sms_orders WHERE id=? AND user_id=?`, id, uid).Scan(&o.ID, &o.UserID, &o.UpstreamID, &o.Country, &o.Service, &o.Phone, &o.Status, &o.Code, &o.UpstreamCost, &o.PriceFen, &o.CreatedAt)
	return o, e
}
...
func (s *Store) ListSMS(uid int64) ([]SMSOrder, error) {
	rows, e := s.DB.Query(`SELECT id,user_id,COALESCE(upstream_id,''),country,service,COALESCE(phone,''),status,COALESCE(code,''),upstream_cost,price_fen,created_at FROM sms_orders WHERE user_id=? ORDER BY created_at DESC LIMIT 100`, uid)
	if e != nil {
		return nil, e
	}
	defer rows.Close()
	...
	for rows.Next() {
		var o SMSOrder
		if e = rows.Scan(&o.ID, &o.UserID, &o.UpstreamID, &o.Country, &o.Service, &o.Phone, &o.Status, &o.Code, &o.UpstreamCost, &o.PriceFen, &o.CreatedAt); e != nil {
			return nil, e
		}
		out = append(out, o)
	}
