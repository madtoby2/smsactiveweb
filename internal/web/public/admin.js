...
async function loadSettings(){const data=await request('/api/admin/settings'),settings=data.settings;Object.keys(settings).forEach(key=>{const input=$(`#${key}`);if(!input)return;if(input.type==='checkbox')input.checked=settings[key]==='true';else input.value=settings[key]});$('#smtpPassword').value='';setConfigStatus('#smtpPasswordStatus',data.smtpPasswordConfigured?'SMTP 密码已配置':'SMTP 密码未配置',data.smtpPasswordConfigured);setConfigStatus('#turnstileStatus',data.turnstileConfigured?'Turnstile 已配置':'Turnstile 未配置',data.turnstileConfigured);}
function setConfigStatus(selector,text,ready){const element=$(selector);element.textContent=text;element.classList.toggle('missing',!ready);}
$('#settingsForm').onsubmit=async event=>{event.preventDefault();const keys=['markupCNY','usdCnyRate','smsmanCnyRate','smtpHost','smtpPort','smtpUser','smtpFrom','contactTitle','contactValue','contactURL','supportHours'],body={};keys.forEach(key=>body[key]=$(`#${key}`).value);body.emailVerificationRequired=String($('#emailVerificationRequired').checked);if($('#smtpPassword').value)body.smtpPassword=$('#smtpPassword').value;await request('/api/admin/settings',{method:'PUT',body:JSON.stringify(body)});$('#settingsResult').textContent='已保存并生效';await loadSettings();setTimeout(()=>$('#settingsResult').textContent='',1800);};
